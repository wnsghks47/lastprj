				...
				<section class="col-md-12">
					<div class="row bodytop_write">
						<div class="col">
							...
							<br>
							<br>
							<h2 style="letter-spacing: 2px; font-size: calc(25px + 0.5vw);">데이터베이스 관리</h2>
					...
					<div class="row my-3">
						<div class="col-12 d-flex justify-content-between mb-3">
							...
							<div>
								<button type="button" class="btn btn-primary" id="insertData" onclick="location.href='InsertProduct'">데이터 입력</button>
								<button type="button" class="btn btn-success" id="insertingredient" onclick="location.href='InsertIngredient'">성분 입력</button>
							</div>
						</div>
						<div class="col-12">
							<table class="table">
								...
								<tbody id="tablePart">
									<c:forEach items="${list }" var="product">
										<tr id="tr${product.pnumber}">
											<td><input type="checkbox" name="deleteList"></td>
											<td>${product.pnumber }</td>
											<td>${product.pbrand }</td>
											<td><a class="text-dark" href="ModifyProduct?n=${product.pnumber }">${product.pname }</a></td>
					...
					<div class="row my-5">
						<div class="col d-flex justify-content-end">
							<form action="ControlData" method="post">
								<div class="d-inline-block mx-3">
									<label for="brand" class="ml-3">브랜드</label>
									<input type="radio" name="topic" id="brand" value="1"/>
									<label for="name" class="ml-3">이름</label>
									<input type="radio" name="topic" id="name" value="2"/>
									<label for="category" class="ml-3">카테고리</label>
									<input type="radio" name="topic" id="category" value="3"/>
								</div>
...
<script>
	$(document).ready(function() {
		...
		/* 선택삭제 버튼 클릭 시 선택한 데이터 일괄 삭제 */
		
		$(document).on('click', '#checkDelete', function() {
			
			var con = confirm('체크된 데이터 목록을 삭제하시겠습니까?');
			...
			if (con == true) {		
				...
				$('input[name=deleteList]:checked').each(function(i){   //jQuery로 for문 돌면서 check 된값 배열에 담는다
					...
					pnumberList.push($(this).closest('td').next().text());
				
				});
		
				
				for (var i = 0; i < pnumberList.length; i++) {
